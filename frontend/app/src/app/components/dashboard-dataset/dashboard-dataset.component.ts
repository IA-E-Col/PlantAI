...
@Component({
  selector: 'app-dashboard-dataset',
  standalone: true,
  imports: [
    CommonModule,
...
export class DashboardDatasetComponent implements OnInit {
  ...
  async collectLibelleClassData() {
    ...
    Object.keys(libelleCount).forEach(libelle => {
      ...
      this.additionalChartTypes.push('bar');
    });
  }
  ...
  renderAdditionalCharts() {
    try {
      ...
      // Format chart titles and update loading state
      try {
        this.getFormattedChartTitles();
      } catch (error) {
        console.error('Error formatting chart titles:', error);
      }
  ...
  formatChartLabel(chartLabel: string): string {
    const formattedLabel = chartLabel.replace(/([A-Z])/g, ' $1').trim();
    ...
    const translations: { [key: string]: string } = {
      ...
      epitheteSpecifique: 'Specific epithet',
      lieu: 'Location',
  ...
  getFormattedChartTitles(): string[] {
    return this.chartData.map(data => this.formatChartLabel(data.chartLabel));
  }
